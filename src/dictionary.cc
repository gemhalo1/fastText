...
//    }
    ...
    int32_t Dictionary::getLine(
        ...
        getLine(in, _words, _labels);

        for (std::string& token : _words) {
    ...
    int32_t Dictionary::getLine(
        ...
        line.erase(0,line.find_first_not_of(" \r\t\v\f"));
        line.erase(line.find_last_not_of(" \r\t\v\f") + 1);

        size_t length = line.length();
        std::string label;

        size_t i_start = 0, i_end = 0;
        while (i_start < length && is_space(line[i_start]))
            i_start++;

        for (i_end = i_start; i_end < length; i_end++) {
            if (is_space(line[i_end])) {
                break;
            }
        }

        if (i_end > i_start) {
            label = line.substr(i_start, i_end - i_start);
            labels.push_back(label);
//            std::cout << label << " ==> ";
        }

        while (i_end < length && is_space(line[i_end]))
            i_end++;

        size_t n_char = 0;
        for (; i_end < length; i_end++) {
            if ((line[i_end] & 0xC0) != 0x80) {

                if(n_char > 0) {
                    words.push_back(line.substr(i_end - n_char, n_char));
                }
